...
- [ ] После каждого перехода активная ссылка имеет понятное состояние.
...
- [ ] Проверить toast при rate-limit fallback.
- [ ] Проверить toast при auth-error fallback.
- [ ] Проверить закрытие toast.
- [ ] Проверить, что toast не перекрывает основной CTA.
...
- [ ] Увидеть кнопку продолжения курса, если она предусмотрена для пустого
  профиля.
- [ ] Убедиться, что отсутствующие review/error-hunt/duel кнопки не занимают
...
- [ ] Засеять два due skills.
- [ ] Проверить interleave review queue из двух навыков.
- [ ] Проверить error-hunt section при достаточном error profile.
- [ ] Запустить error hunt.
- [ ] Проверить leech section при наличии leech item.
- [ ] Засеять пару passed skills.
- [ ] Проверить появление Contrast Duel.
- [ ] Запустить Contrast Duel.
- [ ] Проверить, что непрошедшая пара не показывает кнопку дуэли.
- [ ] Проверить Continue Course.
...
- [ ] Проверить отсутствие доступной listening-сессии без попыток.
...
- [ ] Проверить все layout viewport из раздела 10.
...
- [ ] Проверить progress counter и корректное уменьшение после перехода.
...
- [ ] При настроенном mock Judge проверить `Ошибки`.
- [ ] Проверить, что `Ошибки` раскрывает эталон и требует REWRITE.
- [ ] Проверить `Разбор`.
...
- [ ] Проверить TutorPanel и ответ mock provider.
- [ ] Открыть TutorChat.
...
- [ ] Открыть `/session` напрямую без pending session.
...
- [ ] Settings: выбрать UK voice.
- [ ] Settings: выбрать female voice.
- [ ] Settings: выбрать male voice.
- [ ] Settings: выбрать 0.7×/0.85×/1×.
- [ ] Проверить preview voice.
...
- [-] Export all Dexie data: функция отсутствует в текущем UI.
- [-] Import JSON backup: функция отсутствует в текущем UI.
- [-] Reset progress: функция отсутствует в текущем UI.
- [ ] Когда функция появится: проверить overwrite semantics.
- [ ] Когда функция появится: проверить ключи не экспортируются default.
- [ ] Когда функция появится: проверить старый backup и Dexie migration.
...
- [ ] Начать разговор.
- [ ] Закрыть/перезагрузить страницу до finish.
- [ ] Вернуться в Free Talk.
- [ ] Увидеть resume prompt.
- [ ] Возобновить разговор.
- [ ] Завершить разговор и убедиться, что transcript не потерян.
...
- [ ] Course map screenshot inspected.
- [ ] Drill screenshot inspected.
- [ ] Review screenshot inspected.
- [ ] Settings screenshot inspected.
- [ ] Mobile layout screenshot inspected.
...
- [ ] Successful first provider.
- [ ] First provider timeout → second provider.
- [ ] First provider 429 → second provider.
- [ ] First provider 401/403 → second provider with auth notice.
...
- [ ] No real provider endpoint is requested.
...
|---|---|---|---|
| 2026-07-17 | Settings | В интерфейсе нет Data/export/import/reset section, хотя это есть в нормативном плане | Ожидает отдельной реализации |
...
- [x] `npm run test` — 46 files, 406 tests passed.
- [x] `npm run build` — exit code 0; production preview получает свежий build через Playwright webServer.
- [x] `npm run e2e` — 125/125 passed: Chromium, Firefox, WebKit, Edge, mobile Chromium, mobile WebKit.
...
- [x] `npm run verify` — exit code 0; 406 unit tests and 125 E2E tests, full order lint → typecheck → unit → build → E2E.
...
- [x] Persistence/isolation — clean browser state fixture, fresh context, IndexedDB reload persistence and language persistence passed.
...
- [x] Playwright failure artifacts exist under `app/playwright-report/` and `app/test-results/`; failed runs produced trace, screenshot and video.
- [!] Remote GitHub Actions jobs have not been executed from this workspace; workflow configuration is ready for cross-browser, OS smoke and artifacts.
...
- [-] Settings data export/import/reset UI is not implemented.
